...
if __name__ == "__main__":
    ...
    vae_checkpointer = ModelCheckpoint(
        filepath=f"{options.data}.vae_weights.hdf5", verbose=1, save_best_only=True
    )
    vae_opt = Adam(lr=learnrate, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    vae.add_loss(
        vae_loss(
            x=vae.input,
    ...
    edis_checkpointer = ModelCheckpoint(
        filepath=f"{options.data}.edis_weights.hdf5", verbose=1, save_best_only=True
    )
    edis.compile(optimizer=Adam(lr=1e-4), loss="binary_crossentropy")
